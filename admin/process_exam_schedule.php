...
if (isset($_GET['action']) && $_GET['action'] === 'download_csv' && isset($_GET['schedule_id'])) {
    ...
    // Get assigned applicants with their scores
    $stmt = $conn->prepare("
        SELECT 
            u.control_number,
            u.first_name,
    ...
    // Write header row
    fputcsv($output, ['Control Number', 'First Name', 'Last Name', 'Exam Date', 'Stanine Score']);
    ...
    foreach ($applicants as $applicant) {
        fputcsv($output, [
            ...
            $applicant['last_name'],
            date('Y-m-d', strtotime($applicant['exam_date'])),
